...
router.get('/patreon/callback', async (req, res) => {
  ...
  const { clientId, clientSecret, redirectUri, tierIds, adminIds, stratosUrl } = config.patreon;
  ...
  try {
    ...
    const entitledTierIds = (identity.included || [])
      ...
      .map(item => item.id);
    const hasAccess = adminIds.includes(identity.data.id) || entitledTierIds.some(id => tierIds.includes(id));
